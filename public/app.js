  ...
  app.innerHTML = `${pageHead("Evidence workspace", "Verify a claim. <em>Trace the evidence.</em>", "Fact-Check first identifies the relevant source categories, then runs a separate evidence search only inside those approved domains. The final answer stays short and linked.")}
    <section class="page checker-layout">
      <form class="checker-card glass-card reveal" id="checker-form">
        ...
        <textarea id="claim" name="claim" maxlength="1800" placeholder="Example: O'zbekistonda naqd pul ishlatish mumkin emasmi?"></textarea>
        ...
        <div class="example-row" aria-label="Example claims"><button class="example-btn" type="button" data-example="O'zbekistonda naqd pul ishlatish mumkin emasmi?">Cash in Uzbekistan</button><button class="example-btn" type="button" data-example="Did a hurricane make landfall in the United States yesterday?">Hurricane report</button><button class="example-btn" type="button" data-example="Is this health claim supported by official public-health sources?">Health claim</button></div>
      </form>
...
function renderMethodV2() {
  app.innerHTML = `${pageHead("The Process", "Two AI steps. <em>One enforced boundary.</em>", "Fact-Check separates source routing from evidence searching so the system cannot answer first and justify itself later.")}
    <section class="page method-grid">
      <div><article class="method-card glass-card reveal"><span class="eyebrow">Step 01 · Category routing</span><h2>Decide where to look before looking.</h2><p>The first request receives the claim and the fixed registry taxonomy. It selects the smallest relevant set of categories — for example, economy and finance plus government and law for a question about cash use in Uzbekistan.</p><p>That routing request has no web-search tool and is not allowed to produce a fact-check verdict.</p></article><article class="method-card glass-card reveal"><span class="eyebrow">Step 02 · Evidence search</span><h2>Search only the selected domain set.</h2><p>A new request receives the selected source catalogue and a server-enforced allowed-domain filter. Returned citations are checked against the same selected domains before they are shown.</p><p>Each evidence search is capped at 100 approved domains. If the selected category set is larger, the platform reports that boundary instead of widening the search.</p></article></div>
