...
function validateAdministratorInput({ email, password }) {
  ...
  if (cleanPassword.length < 8 || cleanPassword.length > 256) throw new Error("ADMIN_PASSWORD must be between 8 and 256 characters.");
...
export function createAuth({ pool, adminEmail = "", adminUsername = "", adminPassword = "", production = false } = {}) {
  ...
  async function currentUser(request) {
    ...
    const result = await pool.query(
      "SELECT id, name, email, created_at FROM app_users WHERE LOWER(email) = LOWER($1) AND role = 'admin' LIMIT 1",
      [envEmail],
    );
...
export function createUserAuth({ pool, production = false } = {}) {
  ...
  return {
    ...
    async login({ email, password }) {
      ...
      if (user.role === "admin") return { administrator: true, user: publicUser(user) };
