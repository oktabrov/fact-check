...
export function trustedSourcesPdf({ sources, version, updatedAt }) {
  const lines = [
    "Fact-Check — Public Source Registry",
    `Registry version ${version} · Updated ${new Date(updatedAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" })}`,
    "",
    "This directory lists public source entries. A listed source does not make a claim true, and entries with a source-use review pending are not used for automated evidence search.",
    "Fact-Check links to original material and does not republish source text, images, logos, or database copies. Where a source-use review is complete, its official policy or licence is listed below.",
    "",
  ];

  sources.forEach((source, index) => {
    const categories = Array.isArray(source.categoryLabels) && source.categoryLabels.length
      ? source.categoryLabels.join(" / ")
      : source.category;
    lines.push(`${String(index + 1).padStart(3, "0")}. ${source.name} [${categories}]`);
    ...
    if (source.usagePolicyUrl && source.usageStatus !== "legacy-review-pending") {
      ...
      lines.push("     Source-use review: pending — not used for automated evidence search.");
    }
    lines.push("");
  });
