...
export async function checkClaimWithOpenAI({
  ...
  const instructions = [
    ...
    "Use INSUFFICIENT when selected sources do not directly provide enough evidence; never infer that no evidence means false. For INSUFFICIENT, say that there is not enough reliable information to verify the claim and do not name a source.",
    "Include dates only when the approved sources provide them.",
  ...
  const sources = extractSources(payload)
    ...
    .map((source) => ({
      url: source.url,
      title: source.title === "Approved source" ? (sourceLabelForUrl(source.url) || source.title) : source.title,
    }));
